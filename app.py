...
with tab_single_pred:
    ...
    if 'input_key_suffix' not in st.session_state:
        st.session_state.input_key_suffix = str(uuid.uuid4())  # Unique suffix for widgets
    ...
    if default_data is None:
        ...
        # Update session state when input mode changes
        if input_mode != st.session_state.input_mode:
            st.session_state.input_mode = input_mode
            # Reset sample index and input key suffix when mode changes
            st.session_state.selected_sample_index = None
            st.session_state.input_key_suffix = str(uuid.uuid4())
            if input_mode == "Manual Input":
                if default_data is not None and len(default_data) >= 28:
                    sample = default_data.loc[27, feature_names]  # Sample 28
                    st.session_state.sample_values = {key: sample[key] if not pd.isna(sample[key]) else np.nan for key in feature_names}
                    st.session_state.missing_values = {key: pd.isna(sample[key]) for key in feature_names}
                else:
                    st.session_state.sample_values = {key: round(float(np.mean(inputs_info[key]["range"])), 2) for key in feature_names}
        ...
        if st.session_state.input_mode == "Load from Dataset":
            ...
            if 0 <= selected_index < num_samples:
                if st.session_state.selected_sample_index != selected_index:
                    # Update session state when a new sample is selected
                    st.session_state.selected_sample_index = selected_index
                    st.session_state.input_key_suffix = str(uuid.uuid4())  # Refresh widget keys
            ...
            for i, key in enumerate(physical_props):
                ...
                with col1 if i % 2 == 0 else col2:
                    is_missing = st.checkbox(
                        ...
                        key=f"missing_{key}_{st.session_state.input_key_suffix}"
                    )
                    if is_missing:
                        inputs[key] = np.nan
                        ...
                        inputs[key] = st.number_input(
                            ...
                            step=0.1,
                            format="%.2f",
                            key=f"input_{key}_{st.session_state.input_key_suffix}",
                            help=f"Range: {info['range'][0]} - {info['range'][1]} {info['unit']}"
                        )
                    # Update session state with user input
                    st.session_state.missing_values[key] = is_missing
                    st.session_state.sample_values[key] = inputs[key] if not is_missing else np.nan
